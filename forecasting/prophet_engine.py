...
from core.exceptions import ForecastError, DataNotFoundError, ValidationError
...
class ProphetForecaster:
    ...
    def __init__(self, parameter_name: str, geographic_code: str):
        ...
        self.logger.info(f"Initialized ProphetForecaster for {parameter_name} ({geographic_code})")
    ...
    def load_historical_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if not data_points:
                raise ValueError(f"No historical data found for {self.parameter_name} in {self.geographic_code}")
            ...
            print(f"Loaded {len(self.historical_data)} data points for {self.parameter_name} ({self.geographic_code})")
            print(f"Date range: {self.historical_data['ds'].min().strftime('%Y-%m-%d')} to {self.historical_data['ds'].max().strftime('%Y-%m-%d')}")
    ...
    def fit_model(self, yearly_seasonality: bool = True, weekly_seasonality: bool = False, 
    ...
    def validate_model(self, holdout_years: int = 3) -> ValidationResult:
        ...
        if len(self.historical_data) < holdout_years + 3:
            print(f"Warning: Not enough data for {holdout_years}-year holdout validation")
    ...
    def generate_forecast(self, horizon_years: int = 5) -> ProphetForecastResult:
        ...
        try:
            ...
            future = self.fitted_model.make_future_dataframe(periods=horizon_years, freq='Y')
            ...
            # Trend information
            trend_info = {
                'overall_trend': 'increasing' if forecast_values[-1] > forecast_values[0] else 'decreasing',
    ...
    def plot_forecast(self, forecast_result: ProphetForecastResult, save_path: Optional[str] = None) -> None:
        """
        ...
        plt.plot(historical_dates, historical_values, 'b-', linewidth=2, label='Historical Data', alpha=0.8)
        ...
        plt.plot(connected_dates, connected_values, 'r-', linewidth=2, label='Forecast', alpha=0.8)
        
        # Add uncertainty intervals (connected to historical)
        plt.fill_between(connected_dates, connected_lower, connected_upper, 
                        color='red', alpha=0.2, label='95% Confidence Interval')
        
        # Formatting
        plt.title(f'Prophet Forecast: {forecast_result.parameter_name} ({forecast_result.geographic_code})', 
    ...
    def run_complete_forecast(self, horizon_years: int = 5, create_plot: bool = True, 
                            save_plot_path: Optional[str] = None) -> ProphetForecastResult:
        """
        ...
        for i, (date, value) in enumerate(zip(forecast_result.forecast_dates, forecast_result.forecast_values)):
        ...
        if create_plot:
            if save_plot_path is None:
                ...
                save_plot_path = plots_dir / f"{self.parameter_name}_{self.geographic_code}_prophet_forecast.png"
...
class ProFormaProphetEngine:
    ...
    def generate_forecasts_for_msa(self, msa_code: str, horizon_years: int = 5) -> Dict[str, ProphetForecastResult]:
        """
        ...
        for metric_name in self.metrics_list:
            try:
                ...
                if metric_name in ['treasury_10y', 'commercial_mortgage_rate', 'fed_funds_rate']:
                    geography = 'NATIONAL'
                else:
                    geography = msa_code
    ...
    def get_forecast_values_for_monte_carlo(self, forecasts: Dict[str, ProphetForecastResult], 
                                          target_year: int = 1) -> Dict[str, float]:
        """
        ...
        for metric_name, forecast_result in forecasts.items():
            if target_year <= len(forecast_result.forecast_values):
                value = forecast_result.forecast_values[target_year - 1]  # Convert to 0-based index
                monte_carlo_inputs[metric_name] = value
            else:
                print(f"Warning: {metric_name} forecast doesn't have {target_year} years of data")
