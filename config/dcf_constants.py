...
class FinancialCalculationConstants:
    ...
    RENOVATION_COST_PER_UNIT: float = (
        10000.0  # Cost per unit for renovation CapEx estimation
    )
    POST_RENOVATION_RENT_MULTIPLIER: float = (
        1.125  # 12.5% rent increase after renovation
    )
...
def validate_constants() -> bool:
    ...
    try:
        ...
        assert (
            RISK_THRESHOLDS.LOW_EQUITY_MULTIPLE_THRESHOLD
            < RISK_THRESHOLDS.STRONG_EQUITY_MULTIPLE_THRESHOLD
        )
        assert (
            RISK_THRESHOLDS.TIGHT_DSCR_THRESHOLD < RISK_THRESHOLDS.STRONG_DSCR_THRESHOLD
        )

        # Validate investment thresholds
        assert (
            INVESTMENT_THRESHOLDS.NEGATIVE_NPV_THRESHOLD
            < INVESTMENT_THRESHOLDS.POSITIVE_NPV_THRESHOLD
        )
        assert (
            INVESTMENT_THRESHOLDS.LOW_IRR_THRESHOLD
            < INVESTMENT_THRESHOLDS.EXCELLENT_IRR_THRESHOLD
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Renovation Cost per Unit: ${FINANCIAL_CONSTANTS.RENOVATION_COST_PER_UNIT:,.0f}"
    )
    print(
        f"Post-Renovation Rent Increase: {(FINANCIAL_CONSTANTS.POST_RENOVATION_RENT_MULTIPLIER - 1) * 100:.1f}%"
    )
    print(f"Default Expense Ratio: {FINANCIAL_CONSTANTS.DEFAULT_EXPENSE_RATIO:.1%}")
    print(
        f"Annual Principal Paydown: {FINANCIAL_CONSTANTS.ANNUAL_PRINCIPAL_PAYDOWN_RATE:.1%}"
    )
    print(f"Low IRR Threshold: {RISK_THRESHOLDS.LOW_IRR_THRESHOLD:.1%}")
    print(
        f"Strong Buy NPV Threshold: ${INVESTMENT_THRESHOLDS.STRONG_POSITIVE_NPV_THRESHOLD:,.0f}"
    )
