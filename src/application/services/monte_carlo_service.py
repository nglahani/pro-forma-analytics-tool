...
class MonteCarloApplicationService:
    ...
    def validate_simulation_quality(
        ...
        first_scenario = simulation_result.scenarios[0] if simulation_result.scenarios else None
        if first_scenario:
        ...
        if simulation_result.correlation_matrix:
            ...
            results['correlation_matrix_valid'] = not simulation_result.request.use_correlations
        ...
        if simulation_result.scenarios:
            ...
            growth_std = float(sum((x - sum(growth_scores)/len(growth_scores))**2 
                                 for x in growth_scores) / len(growth_scores))**0.5
