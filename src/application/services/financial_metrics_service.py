...
from src.domain.entities.financial_metrics import FinancialMetrics, TerminalValue, CashFlowSummary, InvestmentRecommendation, RiskLevel
...
from config.dcf_constants import FINANCIAL_CONSTANTS, RISK_THRESHOLDS, INVESTMENT_THRESHOLDS
...
class FinancialMetricsService:
    ...
    def calculate_financial_metrics(self, 
                                  ...
                                  discount_rate: float = 0.10) -> FinancialMetrics:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating financial metrics for property {cash_flow_projection.property_id}, "
                           f"scenario {cash_flow_projection.scenario_id}")
            
            # Calculate terminal value
            terminal_value = self._calculate_terminal_value(cash_flow_projection, dcf_assumptions, initial_numbers)
            
            # Prepare cash flows for analysis
            cash_flow_summary = self._prepare_cash_flows(cash_flow_projection, initial_numbers, terminal_value)
            ...
            modified_irr = self._calculate_modified_irr(cash_flow_summary.total_cash_flows, discount_rate)
            ...
            equity_multiple = total_return / cash_flow_summary.initial_investment if cash_flow_summary.initial_investment > 0 else 0
            
            # Calculate other metrics
            cash_on_cash_year_1 = cash_flow_summary.annual_cash_flows[0] / cash_flow_summary.initial_investment if len(cash_flow_summary.annual_cash_flows) > 0 and cash_flow_summary.initial_investment > 0 else 0
            payback_period = self._calculate_payback_period(cash_flow_summary.total_cash_flows)
            average_annual_return = self._calculate_average_annual_return(cash_flow_summary.annual_cash_flows, cash_flow_summary.initial_investment)
            
            # Calculate risk metrics
            debt_service_coverage_avg = self._calculate_average_dscr(cash_flow_projection)
            ltv_ratio = initial_numbers.calculate_ltv_ratio()
            cap_rate_year_1 = initial_numbers.initial_cap_rate
            
            # Calculate break-even analysis
            break_even_analysis = self._calculate_break_even_analysis(cash_flow_projection, initial_numbers)
            
            # Determine risk level and investment recommendation
            risk_level = self._assess_risk_level(irr, equity_multiple, debt_service_coverage_avg, ltv_ratio)
            investment_recommendation, rationale = self._generate_investment_recommendation(
            ...
            financial_metrics = FinancialMetrics(
                ...
                return_on_investment=(total_return - cash_flow_summary.initial_investment) / cash_flow_summary.initial_investment if cash_flow_summary.initial_investment > 0 else 0,
            ...
            self.logger.info(f"Successfully calculated financial metrics: "
            ...
            raise ValidationError(f"Financial metrics calculation failed: {str(e)}") from e
    
    def _calculate_terminal_value(self, cash_flow_projection: CashFlowProjection,
                                dcf_assumptions: DCFAssumptions,
                                initial_numbers: InitialNumbers) -> TerminalValue:
        ...
        if not year_5_cash_flow:
            raise ValidationError("Year 5 cash flow not found for terminal value calculation")
    ...
    def _prepare_cash_flows(self, cash_flow_projection: CashFlowProjection,
                           initial_numbers: InitialNumbers,
                           terminal_value: TerminalValue) -> CashFlowSummary:
        ...
        for year in range(1, 6):  # Years 1-5
            ...
            if distribution:
                ...
                total_year_distribution = distribution.investor_cash_distribution + distribution.operator_cash_distribution
    ...
    def _calculate_irr(self, cash_flows: List[float], 
                      max_iterations: int = None, precision: float = None) -> float:
        ...
        for iteration in range(max_iterations):
            ...
            for i, cash_flow in enumerate(cash_flows):
                if i == 0:
                    ...
                    npv_derivative -= i * cash_flow / (discount_factor * (1 + irr_guess))
    ...
    def _calculate_modified_irr(self, cash_flows: List[float], discount_rate: float) -> float:
    ...
    def _calculate_average_annual_return(self, annual_cash_flows: List[float], initial_investment: float) -> float:
    ...
    def _calculate_average_dscr(self, cash_flow_projection: CashFlowProjection) -> float:
    ...
    def _calculate_break_even_analysis(self, cash_flow_projection: CashFlowProjection,
                                     initial_numbers: InitialNumbers) -> Dict[str, float]:
        ...
        fixed_costs = year_1_cash_flow.total_operating_expenses + year_1_cash_flow.annual_debt_service
        gross_potential_income = year_1_cash_flow.gross_rental_income / (1 - year_1_cash_flow.vacancy_loss / year_1_cash_flow.gross_rental_income) if year_1_cash_flow.gross_rental_income > 0 else 0
        break_even_occupancy = fixed_costs / gross_potential_income if gross_potential_income > 0 else 1.0
        
        # Break-even rent (per unit per month)
        total_units = initial_numbers.residential_units.total_units if hasattr(initial_numbers, 'residential_units') else 20  # Estimate
        break_even_monthly_rent = fixed_costs / (12 * total_units) if total_units > 0 else 0
        
        # Break-even cap rate
        break_even_cap_rate = year_1_cash_flow.net_operating_income / initial_numbers.purchase_price if initial_numbers.purchase_price > 0 else 0
        ...
        return {
            ...
            'current_occupancy_rate': 1.0 - (year_1_cash_flow.vacancy_loss / year_1_cash_flow.gross_rental_income) if year_1_cash_flow.gross_rental_income > 0 else 0
        }
    
    def _assess_risk_level(self, irr: float, equity_multiple: float, avg_dscr: float, ltv_ratio: float) -> RiskLevel:
    ...
    def _generate_investment_recommendation(self, npv: float, irr: float, equity_multiple: float,
                                          payback_period: float, risk_level: RiskLevel) -> Tuple[InvestmentRecommendation, str]:
    ...
    def get_metrics_summary(self, financial_metrics: FinancialMetrics) -> Dict[str, Any]:
    ...
    def compare_scenarios(self, metrics_list: List[FinancialMetrics]) -> Dict[str, Any]:
        ...
        sorted_by_npv = sorted(metrics_list, key=lambda m: m.net_present_value, reverse=True)
        
        # Sort by IRR (descending)
        sorted_by_irr = sorted(metrics_list, key=lambda m: m.internal_rate_return, reverse=True)
        ...
        return {
            ...
            'recommendations': {
                rec.value: len([m for m in metrics_list if m.investment_recommendation == rec])
...
def calculate_financial_metrics(cash_flow_projection: CashFlowProjection,
                               dcf_assumptions: DCFAssumptions,
                               initial_numbers: InitialNumbers,
                               discount_rate: float = 0.10) -> FinancialMetrics:
    """Convenience function to calculate financial metrics."""
    service = FinancialMetricsService()
    return service.calculate_financial_metrics(cash_flow_projection, dcf_assumptions, initial_numbers, discount_rate)