...
class CashFlowProjectionService:
    ...
    def calculate_cash_flow_projection(self, 
                                     dcf_assumptions: DCFAssumptions,
                                     initial_numbers: InitialNumbers) -> CashFlowProjection:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating cash flow projection for property {initial_numbers.property_id}, "
                           f"scenario {dcf_assumptions.scenario_id}")
            
            # Calculate annual cash flows for Years 0-5
            annual_cash_flows = []
            for year in range(6):
                cash_flow = self._calculate_annual_cash_flow(year, dcf_assumptions, initial_numbers)
            ...
            for year in range(6):
                distribution, cumulative_unpaid_preferred = self._calculate_waterfall_distribution(
                    year, annual_cash_flows[year], dcf_assumptions, initial_numbers, cumulative_unpaid_preferred
                )
            ...
            self.logger.info(f"Successfully calculated cash flow projection: "
            ...
            raise ValidationError(f"Cash flow projection calculation failed: {str(e)}") from e
    
    def _calculate_annual_cash_flow(self, year: int, 
                                  dcf_assumptions: DCFAssumptions,
                                  initial_numbers: InitialNumbers) -> AnnualCashFlow:
        ...
        if year == 0:
            ...
            if year == 1:
                ...
                if hasattr(initial_numbers, 'pre_renovation_annual_rent') and initial_numbers.pre_renovation_annual_rent > 0:
                    # Estimate renovation months from the year 1 calculation
                    renovation_months = 12 - (initial_numbers.year_1_rental_income / initial_numbers.post_renovation_annual_rent * 12)
                ...
                for y in range(1, year + 1):
                    year_growth = dcf_assumptions.get_year_assumptions(y)['rent_growth_rate']
        ...
        if year == 0:
            ...
            for y in range(1, year + 1):
                year_growth = dcf_assumptions.get_year_assumptions(y)['expense_growth_rate']
                expense_growth_compound *= (1 + year_growth)
            
            operating_expenses = {
                'property_taxes': initial_numbers.property_taxes * expense_growth_compound,
                'insurance': initial_numbers.insurance * expense_growth_compound,
                'repairs_maintenance': initial_numbers.repairs_maintenance * expense_growth_compound,
                'property_management': initial_numbers.property_management * expense_growth_compound,
                'admin_expenses': initial_numbers.admin_expenses * expense_growth_compound,
                'contracting': initial_numbers.contracting * expense_growth_compound,
                'replacement_reserves': initial_numbers.replacement_reserves * expense_growth_compound
            }
    ...
    def _calculate_waterfall_distribution(self, year: int,
                                        annual_cash_flow: AnnualCashFlow,
                                        dcf_assumptions: DCFAssumptions,
                                        initial_numbers: InitialNumbers,
                                        cumulative_unpaid_preferred: float) -> tuple[WaterfallDistribution, float]:
        ...
        if year == 0:
            ...
            investor_preferred_return_due = investor_cash_invested * preferred_return_rate
        
        # Add any unpaid preferred return from previous years
        total_preferred_due = investor_preferred_return_due + cumulative_unpaid_preferred
        ...
        investor_preferred_return_accrued = max(0.0, investor_preferred_return_due - investor_preferred_return_paid)
        new_cumulative_unpaid_preferred = max(0.0, cumulative_unpaid_preferred + investor_preferred_return_due - investor_preferred_return_paid)
        ...
        if remaining_after_preferred > 0:
            investor_equity_distribution = remaining_after_preferred * dcf_assumptions.investor_equity_share
            operator_equity_distribution = remaining_after_preferred * (1 - dcf_assumptions.investor_equity_share)
        else:
            investor_equity_distribution = 0.0
            operator_equity_distribution = 0.0
        
        # Total distributions
        investor_cash_distribution = investor_preferred_return_paid + investor_equity_distribution
    ...
    def validate_cash_flow_projection(self, projection: CashFlowProjection) -> List[str]:
        ...
        try:
            ...
            for year in range(1, 6):
                ...
                if cash_flow and cash_flow.net_operating_income < 0:
                    issues.append(f"Negative NOI in Year {year}: ${cash_flow.net_operating_income:,.0f}")
            
            # Check for extremely high expense ratios
            performance_metrics = projection.get_performance_metrics()
            if performance_metrics.get('average_expense_ratio', 0) > 0.6:
                issues.append(f"High expense ratio: {performance_metrics['average_expense_ratio']:.1%}")
            
            # Check for unpaid preferred returns
            final_distribution = projection.get_distribution_by_year(5)
            if final_distribution and final_distribution.cumulative_unpaid_preferred > 1000:
                issues.append(f"Significant unpaid preferred return: ${final_distribution.cumulative_unpaid_preferred:,.0f}")
            ...
            if year_1_cf and year_5_cf and year_1_cf.net_operating_income > 0:
                noi_growth = (year_5_cf.net_operating_income / year_1_cf.net_operating_income) - 1
                if noi_growth > 1.0:  # More than 100% growth
                    ...
                    issues.append(f"Significant NOI decline over 5 years: {noi_growth:.1%}")
    ...
    def calculate_annual_returns(self, projection: CashFlowProjection) -> Dict[int, Dict[str, float]]:
...
def calculate_cash_flow_projection(dcf_assumptions: DCFAssumptions,
                                 initial_numbers: InitialNumbers) -> CashFlowProjection:
