...
class InitialNumbersService:
    ...
    def calculate_initial_numbers(self, 
                                property_data: SimplifiedPropertyInput,
                                dcf_assumptions: DCFAssumptions) -> InitialNumbers:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating initial numbers for property {property_data.property_id}, "
            ...
            closing_cost_amount = dcf_assumptions.calculate_closing_costs(purchase_price)
            ...
            annual_interest_expense = self._calculate_annual_interest(loan_amount, dcf_assumptions)
            lender_reserves_amount = dcf_assumptions.calculate_lender_reserves(annual_interest_expense)
            
            # 3. Equity Requirements
            total_cash_required = self._calculate_total_cash_required(
                purchase_price, loan_amount, closing_cost_amount, 
                renovation_capex, lender_reserves_amount
            )
            investor_cash_required = total_cash_required * dcf_assumptions.investor_equity_share
            operator_cash_required = total_cash_required * (1 - dcf_assumptions.investor_equity_share)
            ...
            after_repair_value = self._calculate_after_repair_value(
                cost_basis, income_calculations['post_renovation_annual_rent'], dcf_assumptions
            )
            ...
            initial_numbers = InitialNumbers(
                ...
                # Income structure
                pre_renovation_annual_rent=income_calculations['pre_renovation_annual_rent'],
                post_renovation_annual_rent=income_calculations['post_renovation_annual_rent'],
                ...
                total_operating_expenses=expense_calculations['total_operating_expenses'],
            ...
            self.logger.info(f"Successfully calculated initial numbers: "
            ...
            raise ValidationError(f"Initial numbers calculation failed: {str(e)}") from e
    ...
    def _get_purchase_price(self, property_data: SimplifiedPropertyInput) -> float:
        ...
        if not property_data.purchase_price or property_data.purchase_price <= 0:
            raise ValidationError("Property purchase price is required and must be positive")
        return float(property_data.purchase_price)
    
    def _calculate_renovation_capex(self, property_data: SimplifiedPropertyInput) -> float:
        """Calculate renovation capital expenditures."""
        # If renovation cost is specified, use it
        if hasattr(property_data.renovation_info, 'estimated_cost') and property_data.renovation_info.estimated_cost:
        ...
        estimated_capex = total_units * FINANCIAL_CONSTANTS.RENOVATION_COST_PER_UNIT * (renovation_months / 12)
        
        self.logger.info(f"Estimated renovation CapEx: ${estimated_capex:,.0f} "
                        f"({total_units} units, {renovation_months} months)")
        
        return estimated_capex
    
    def _calculate_annual_interest(self, loan_amount: float, dcf_assumptions: DCFAssumptions) -> float:
    ...
    def _calculate_total_cash_required(self, purchase_price: float, loan_amount: float,
                                     closing_costs: float, renovation_capex: float,
                                     lender_reserves: float) -> float:
    ...
    def _calculate_income_structure(self, property_data: SimplifiedPropertyInput) -> Dict[str, float]:
        ...
        monthly_commercial_rent = (property_data.commercial_units.monthly_gross_rent 
                                 if property_data.commercial_units else 0.0)
        ...
        post_renovation_annual_rent = pre_renovation_annual_rent * post_renovation_multiplier
        ...
        year_1_rental_income = post_renovation_annual_rent * (12 - renovation_months) / 12
        
        self.logger.info(f"Income calculations: Pre-reno ${pre_renovation_annual_rent:,.0f}, "
    ...
    def _calculate_operating_expenses(self, property_data: SimplifiedPropertyInput, 
                                    annual_rent: float) -> Dict[str, float]:
        ...
        expense_ratio = expenses['total_operating_expenses'] / annual_rent if annual_rent > 0 else 0
        
        self.logger.info(f"Operating expenses: ${expenses['total_operating_expenses']:,.0f} "
                        f"({expense_ratio:.1%} of gross rent)")
        
        return expenses
    
    def _calculate_after_repair_value(self, cost_basis: float, annual_rent: float, 
                                    dcf_assumptions: DCFAssumptions) -> float:
        ...
        self.logger.info(f"After-repair value: ${arv:,.0f} "
                        f"(NOI ${estimated_noi:,.0f} / Cap Rate {year_1_cap_rate:.1%})")
        
        return arv
    
    def _calculate_initial_cap_rate(self, year_1_income: float, operating_expenses: float,
    ...
    def get_calculation_summary(self, initial_numbers: InitialNumbers) -> Dict[str, Any]:
    ...
    def validate_initial_numbers(self, initial_numbers: InitialNumbers) -> list:
        ...
        try:
            ...
            if initial_numbers.post_renovation_annual_rent > 0:
                expense_ratio = initial_numbers.total_operating_expenses / initial_numbers.post_renovation_annual_rent
            ...
            if initial_numbers.initial_cap_rate < 0.03:
                issues.append(f"Low initial cap rate: {initial_numbers.initial_cap_rate:.1%}")
            elif initial_numbers.initial_cap_rate > 0.15:
                issues.append(f"High initial cap rate: {initial_numbers.initial_cap_rate:.1%}")
...
def calculate_initial_numbers(property_data: SimplifiedPropertyInput, 
                            dcf_assumptions: DCFAssumptions) -> InitialNumbers:
