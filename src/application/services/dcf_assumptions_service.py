...
class DCFAssumptionsService:
    ...
    def create_dcf_assumptions_from_scenario(self, 
                                           monte_carlo_scenario: Dict[str, Any],
                                           property_data: SimplifiedPropertyInput) -> DCFAssumptions:
        """
        ...
        try:
            ...
            if validation_issues:
                raise ValidationError(f"Monte Carlo validation failed: {'; '.join(validation_issues)}")
            
            # Create DCF assumptions
            dcf_assumptions = DCFAssumptions(
                scenario_id=monte_carlo_scenario.get('scenario_id', f"scenario_{date.today().strftime('%Y%m%d')}"),
                msa_code=self._extract_msa_code(property_data),
                property_id=property_data.property_id,
                
                # Map forecasted parameters using our mapping
                commercial_mortgage_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['commercial_mortgage_rate']],
                treasury_10y_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['treasury_10y_rate']],
                fed_funds_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['fed_funds_rate']],
                cap_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['cap_rate']],
                rent_growth_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['rent_growth_rate']],
                expense_growth_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['expense_growth_rate']],
                property_growth_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['property_growth_rate']],
                vacancy_rate=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['vacancy_rate']],
                
                # Static parameters (use first value from forecasted arrays)
                ltv_ratio=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['ltv_ratio']][0],
                closing_cost_pct=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['closing_cost_pct']][0],
                lender_reserves_months=forecasted_params[MONTE_CARLO_PARAMETER_MAPPING['lender_reserves']][0],
            ...
            self.logger.info(f"Created DCF assumptions for scenario {dcf_assumptions.scenario_id}, "
    ...
    def create_dcf_assumptions_batch(self, 
                                   monte_carlo_results: Dict[str, Any],
                                   property_data: SimplifiedPropertyInput) -> List[DCFAssumptions]:
        """
        ...
        try:
            ...
            for i, scenario in enumerate(scenarios):
                try:
                    dcf_assumptions = self.create_dcf_assumptions_from_scenario(scenario, property_data)
            ...
            self.logger.info(f"Created {len(dcf_assumptions_list)} DCF assumptions from "
                           f"{len(scenarios)} scenarios for property {property_data.property_id}")
            
            if not dcf_assumptions_list:
                raise ValidationError("No valid DCF assumptions could be created from Monte Carlo results")
    ...
    def validate_assumptions_compatibility(self, assumptions: DCFAssumptions) -> List[str]:
        """
        ...
        try:
            ...
            for year, rate in enumerate(assumptions.commercial_mortgage_rate):
                if rate < 0.02 or rate > 0.15:
                    issues.append(f"Commercial mortgage rate Year {year}: {rate:.1%} seems unreasonable")
            ...
            for i in range(1, len(cap_rates)):
                ...
                if year_change > 0.02:  # More than 2% change year-over-year
                    issues.append(f"Large cap rate change Year {i-1} to {i}: "
            ...
            for year, growth in enumerate(assumptions.rent_growth_rate):
                if growth < -0.05 or growth > 0.15:
                    issues.append(f"Rent growth Year {year}: {growth:.1%} seems extreme")
            
            for year, growth in enumerate(assumptions.expense_growth_rate):
                if growth < -0.02 or growth > 0.10:
                    issues.append(f"Expense growth Year {year}: {growth:.1%} seems extreme")
            
            # Check investment structure makes sense
            if assumptions.investor_equity_share + (1 - assumptions.investor_equity_share) != 1.0:
            ...
            if debt_financing + cash_financing > 1.1:  # Allow some flexibility
                issues.append(f"LTV ({debt_financing:.1%}) + Cash ({cash_financing:.1%}) > 100%")
    ...
    def _extract_msa_code(self, property_data: SimplifiedPropertyInput) -> str:
        ...
        self.logger.warning(f"Could not determine MSA code for property {property_data.property_id}, "
    ...
    def get_assumption_summary(self, assumptions: DCFAssumptions) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_rent_growth': sum(assumptions.rent_growth_rate[1:]) / 5,  # Years 1-5 average
            ...
            # Risk indicators
            'mortgage_rate_volatility': max(assumptions.commercial_mortgage_rate) - min(assumptions.commercial_mortgage_rate),
            'cap_rate_volatility': max(assumptions.cap_rate) - min(assumptions.cap_rate),
            'max_vacancy_rate': max(assumptions.vacancy_rate)
        }


# Convenience functions for easy usage
def create_dcf_assumptions(monte_carlo_scenario: Dict[str, Any], 
                         property_data: SimplifiedPropertyInput) -> DCFAssumptions:
    """Convenience function to create DCF assumptions."""
    service = DCFAssumptionsService()
    return service.create_dcf_assumptions_from_scenario(monte_carlo_scenario, property_data)


def create_dcf_assumptions_batch(monte_carlo_results: Dict[str, Any],
                               property_data: SimplifiedPropertyInput) -> List[DCFAssumptions]:
