...
class MonteCarloVisualizer:
    ...
    def create_comprehensive_dashboard(
        ...
        try:
            ...
            chart_files['parameter_distributions'] = self._create_parameter_distributions(
                simulation_result, output_path, save_plots, show_plots
            )
            
            # 2. Scenario Classification Analysis
            chart_files['scenario_classification'] = self._create_scenario_classification_chart(
                simulation_result, output_path, save_plots, show_plots
            )
            ...
            if hasattr(simulation_result, 'correlation_matrix') and simulation_result.correlation_matrix is not None:
            ...
            chart_files['statistical_validation'] = self._create_statistical_validation_dashboard(
                simulation_result, output_path, save_plots, show_plots
            )
    ...
    def _create_parameter_distributions(
        ...
        fig.suptitle('Monte Carlo Parameter Distributions with Statistical Analysis', 
        ...
        for idx, (param_name, values) in enumerate(parameter_data.items()):
            ...
            # Create histogram
            n, bins, patches = ax.hist(values, bins=30, alpha=0.7, density=True, 
            ...
            ax.axvline(p50, color='orange', linestyle='--', linewidth=2, label=f'Median: {p50:.3f}')
            ax.axvline(p5, color='red', linestyle=':', alpha=0.7, label=f'5th %: {p5:.3f}')
            ax.axvline(p95, color='red', linestyle=':', alpha=0.7, label=f'95th %: {p95:.3f}')
    ...
    def _create_scenario_classification_chart(
        ...
        fig.suptitle('Monte Carlo Scenario Classification Analysis', fontsize=16, fontweight='bold')
        ...
        ax3.axvline(np.mean(growth_scores), color='red', linestyle='--', linewidth=2, 
        ...
        ax4.axvline(np.mean(risk_scores), color='blue', linestyle='--', linewidth=2,
    ...
    def _create_risk_growth_scatter(
        ...
        for scenario_type in unique_scenarios:
            ...
            ax.scatter(x_vals, y_vals, c=[color_map[scenario_type]], 
        ...
        # Add quadrant labels
        ax.text(0.25, 0.75, 'High Risk\nLow Growth', ha='center', va='center', 
               bbox=dict(boxstyle='round', facecolor='red', alpha=0.3))
        ax.text(0.75, 0.75, 'High Risk\nHigh Growth', ha='center', va='center',
               bbox=dict(boxstyle='round', facecolor='orange', alpha=0.3))
        ax.text(0.25, 0.25, 'Low Risk\nLow Growth', ha='center', va='center',
               bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.3))
        ax.text(0.75, 0.25, 'Low Risk\nHigh Growth', ha='center', va='center',
               bbox=dict(boxstyle='round', facecolor='green', alpha=0.3))
        
        # Formatting
        ax.set_xlabel('Growth Score', fontweight='bold')
        ax.set_ylabel('Risk Score', fontweight='bold')
        ax.set_title('Risk vs Growth Analysis by Market Scenario', fontsize=14, fontweight='bold')
    ...
    def _create_time_series_evolution(
        ...
        if simulation_result.scenarios:
            ...
            available_params = [p for p in key_parameters 
                             if p in first_scenario.forecasted_parameters]
        ...
        fig, axes = plt.subplots(rows, cols, figsize=(16, 4 * rows))
        fig.suptitle('Parameter Evolution Over Time (Monte Carlo Scenarios)', 
                     fontsize=16, fontweight='bold')
        ...
        for idx, param_name in enumerate(available_params):
            ...
            all_scenarios_data = []
            for scenario in simulation_result.scenarios[:50]:  # Limit to 50 scenarios for clarity
                if param_name in scenario.forecasted_parameters:
                    all_scenarios_data.append(scenario.forecasted_parameters[param_name])
            
            # Plot individual scenarios with low alpha
            for scenario_data in all_scenarios_data:
                if len(scenario_data) == len(years):
                    ax.plot(years, scenario_data, color='blue', alpha=0.1, linewidth=0.5)
            ...
            if all_scenarios_data:
                ...
                percentiles_5 = [np.percentile(year_values, 5) for year_values in values_by_year]
                ...
                percentiles_95 = [np.percentile(year_values, 95) for year_values in values_by_year]
                
                # Plot percentile bands
                ax.fill_between(years, percentiles_5, percentiles_95, 
                              alpha=0.2, color='red', label='5th-95th percentile')
                ax.fill_between(years, percentiles_25, percentiles_75, 
                              alpha=0.3, color='orange', label='25th-75th percentile')
                ax.plot(years, percentiles_50, color='red', linewidth=3, 
    ...
    def _create_correlation_heatmap(
        ...
        try:
            ...
            ax.set_xticklabels([name.replace('_', ' ').title() for name in parameter_names], 
                             rotation=45, ha='right')
            ax.set_yticklabels([name.replace('_', ' ').title() for name in parameter_names])
    ...
    def _create_extreme_scenarios_chart(
        ...
        if not hasattr(simulation_result, 'extreme_scenarios') or not simulation_result.extreme_scenarios:
            ...
            if hasattr(scenario, 'forecasted_parameters'):
                ...
                for param in parameter_names:
                    if param in scenario.forecasted_parameters:
                        scenario_averages[param] = np.mean(scenario.forecasted_parameters[param])
        ...
        for i, (scenario_type, color) in enumerate(zip(scenario_types, colors)):
            values = [extreme_data[scenario_type].get(param, 0) for param in available_params]
            bars = ax.bar(x + i * width, values, width, label=scenario_type.replace('_', ' ').title(), 
        ...
        ax.set_xticklabels([param.replace('_', ' ').title() for param in available_params])
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    def _create_statistical_validation_dashboard(
        ...
        fig.suptitle('Monte Carlo Statistical Validation Dashboard', fontsize=16, fontweight='bold')
        
        # 1. Scenario count validation
        ax1 = fig.add_subplot(gs[0, 0])
        expected_scenarios = getattr(simulation_result, 'num_scenarios', len(simulation_result.scenarios))
        actual_scenarios = len(simulation_result.scenarios)
        
        ax1.bar(['Expected', 'Actual'], [expected_scenarios, actual_scenarios], 
               color=['blue', 'green' if actual_scenarios == expected_scenarios else 'red'])
        ...
        if simulation_result.scenarios:
            ...
            ax2.bar(['Expected', 'Actual'], [expected_params, param_count],
                   color=['blue', 'green' if param_count >= expected_params * 0.9 else 'red'])
            ax2.set_title('Parameter Coverage', fontweight='bold')
            ax2.set_ylabel('Number of Parameters')
            for i, v in enumerate([expected_params, param_count]):
                ax2.text(i, v + v*0.01, str(v), ha='center', va='bottom', fontweight='bold')
        
        # 3. Growth score distribution normality test
        ax3 = fig.add_subplot(gs[0, 2])
        growth_scores = [s.scenario_summary.get('growth_score', 0.5) for s in simulation_result.scenarios]
        ...
        risk_scores = [s.scenario_summary.get('risk_score', 0.5) for s in simulation_result.scenarios]
        ...
        ax4.set_title(f'Growth vs Risk Correlation\nr = {correlation:.3f}', fontweight='bold')
        ax4.grid(True, alpha=0.3)
        ...
        ax5.bar(['Diversity Score'], [diversity_score], 
               color='green' if diversity_score > quality_threshold else 'red')
        ax5.axhline(quality_threshold, color='red', linestyle='--', label=f'Threshold: {quality_threshold}')
        ax5.set_title('Scenario Diversity\n(Growth Score Std Dev)', fontweight='bold')
        ax5.set_ylabel('Standard Deviation')
        ax5.legend()
        ax5.text(0, diversity_score + diversity_score*0.1, f'{diversity_score:.4f}', 
                ha='center', va='bottom', fontweight='bold')
        ...
        bars = ax6.bar(scenario_types, counts, color=plt.cm.Set3(np.linspace(0, 1, len(scenario_types))))
