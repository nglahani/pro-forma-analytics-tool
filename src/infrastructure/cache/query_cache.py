...
class QueryCache:
    ...
    def _init_persistent_cache(self) -> None:
        ...
        try:
            with sqlite3.connect(str(self.cache_db_path)) as conn:
                ...
                conn.commit()
                self.logger.info(
                    f"Initialized query cache database at {self.cache_db_path}"
                )
    ...
    def _generate_cache_key(
        self, query: str, params: Tuple = (), extra_context: Optional[Dict] = None
    ) -> str:
                ...
                with sqlite3.connect(str(self.cache_db_path)) as conn:
                    ...
                    if row:
                        ...
                        if not self._is_expired(expires_at):
                            ...
                            self.logger.debug(
                                f"Persistent cache hit for key: {cache_key[:16]}..."
                            )
                            return data
                        else:
                            # Remove expired entry
    ...
    def set(
        ...
        try:
            ...
            self.logger.debug(
                f"Cached query result: {cache_key[:16]}... ({data_size} bytes)"
            )
            return True
