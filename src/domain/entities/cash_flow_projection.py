...
class AnnualCashFlow:
    ...
    def _validate_income_calculations(self):
        ...
        if abs(self.effective_gross_income - expected_egi) > 0.01:
            raise ValidationError(f"Effective gross income ({self.effective_gross_income:,.2f}) "
    ...
    def _validate_expense_calculations(self):
        ...
        expense_components = [
            self.property_taxes, self.insurance, self.repairs_maintenance,
            self.property_management, self.admin_expenses, self.contracting,
        ...
        if abs(self.total_operating_expenses - expected_total) > 0.01:
            raise ValidationError(f"Total operating expenses ({self.total_operating_expenses:,.2f}) "
    ...
    def _validate_cash_flow_calculations(self):
        ...
        if abs(self.net_operating_income - expected_noi) > 0.01:
            raise ValidationError(f"Net operating income ({self.net_operating_income:,.2f}) "
        ...
        if abs(self.before_tax_cash_flow - expected_btcf) > 0.01:
            raise ValidationError(f"Before-tax cash flow ({self.before_tax_cash_flow:,.2f}) "
        ...
        if abs(self.net_cash_flow - expected_ncf) > 0.01:
            raise ValidationError(f"Net cash flow ({self.net_cash_flow:,.2f}) "
    ...
    def _validate_amounts(self):
        ...
        for name, amount in amounts.items():
            if amount < -0.01:  # Allow for small floating point errors
                raise ValidationError(f"Distribution amount '{name}' cannot be negative: {amount}")
    
    def _validate_distribution_calculations(self):
        """Validate distribution calculations are consistent."""
        # Check total distribution
        expected_total = self.investor_cash_distribution + self.operator_cash_distribution
        if abs(self.total_cash_distributed - expected_total) > 0.01:
            raise ValidationError(f"Total distributed ({self.total_cash_distributed:,.2f}) "
        ...
        if abs(self.remaining_cash - expected_remaining) > 0.01:
            raise ValidationError(f"Remaining cash ({self.remaining_cash:,.2f}) "
...
class CashFlowProjection:
    ...
    def _validate_annual_cash_flows(self):
        ...
        for i, cash_flow in enumerate(self.annual_cash_flows):
            if cash_flow.year != i:
                raise ValidationError(f"Cash flow {i} has incorrect year {cash_flow.year}")
    ...
    def _validate_waterfall_distributions(self):
        ...
        if len(self.waterfall_distributions) != 6:
            raise ValidationError("Must have exactly 6 waterfall distributions (Years 0-5)")
        
        for i, distribution in enumerate(self.waterfall_distributions):
            if distribution.year != i:
                raise ValidationError(f"Distribution {i} has incorrect year {distribution.year}")
    
    def _calculate_summary_metrics(self):
        """Calculate summary metrics from annual cash flows."""
        self.total_gross_income = sum(cf.gross_rental_income for cf in self.annual_cash_flows)
        self.total_operating_expenses = sum(cf.total_operating_expenses for cf in self.annual_cash_flows)
        self.total_noi = sum(cf.net_operating_income for cf in self.annual_cash_flows)
        self.total_debt_service = sum(cf.annual_debt_service for cf in self.annual_cash_flows)
        self.total_before_tax_cash_flow = sum(cf.before_tax_cash_flow for cf in self.annual_cash_flows)
        self.total_investor_distributions = sum(wd.investor_cash_distribution for wd in self.waterfall_distributions)
        self.total_operator_distributions = sum(wd.operator_cash_distribution for wd in self.waterfall_distributions)
    
    def get_cash_flow_by_year(self, year: int) -> Optional[AnnualCashFlow]:
        """Get cash flow for a specific year."""
        if not (0 <= year <= 5):
            return None
        return self.annual_cash_flows[year] if year < len(self.annual_cash_flows) else None
    
    def get_distribution_by_year(self, year: int) -> Optional[WaterfallDistribution]:
        """Get waterfall distribution for a specific year."""
        if not (0 <= year <= 5):
            return None
        return self.waterfall_distributions[year] if year < len(self.waterfall_distributions) else None
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        avg_noi = sum(cf.net_operating_income for cf in operational_years) / len(operational_years)
        avg_cash_flow = sum(cf.before_tax_cash_flow for cf in operational_years) / len(operational_years)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CashFlowProjection':
        ...
        calculation_date = date.fromisoformat(data['calculation_date']) if 'calculation_date' in data else date.today()
        
        # Reconstruct annual cash flows
        annual_cash_flows = []
        for cf_data in data.get('annual_cash_flows', []):
            annual_cash_flows.append(AnnualCashFlow(
                year=cf_data['year'],
        ...
        for wd_data in data.get('waterfall_distributions', []):
            waterfall_distributions.append(WaterfallDistribution(
                year=wd_data['year'],
                available_cash=wd_data['available_cash'],
                investor_preferred_return_due=wd_data['investor_preferred_return_due'],
                investor_preferred_return_paid=wd_data['investor_preferred_return_paid'],
                investor_preferred_return_accrued=wd_data['investor_preferred_return_accrued'],
    ...
    def __str__(self) -> str:
        ...
        return (f"CashFlowProjection(property={self.property_id}, scenario={self.scenario_id}, "
