...
class Scenario:
    ...
    def __post_init__(self):
        ...
        year_counts = {param: len(values) for param, values in self.parameter_values.items()}
    ...
    def get_parameter_average(self, parameter_name: str) -> float:
        ...
        return sum(self.parameter_values[parameter_name]) / len(self.parameter_values[parameter_name])
...
class SimulationSummary:
    ...
    parameter_statistics: Dict[str, Dict[str, float]]  # param -> {mean, std, p5, p95, etc.}
...
class SimulationResult:
    ...
    def get_scenario_by_rank(self, percentile: float) -> Scenario:
        ...
        scenarios_with_ranks = [s for s in self.scenarios if s.percentile_rank is not None]
        ...
        closest_scenario = min(
            ...
            key=lambda s: abs(s.percentile_rank - percentile)
        )
        return closest_scenario
    
    def get_scenarios_by_market_type(self, market_scenario: MarketScenario) -> List[Scenario]:
        """Get all scenarios of a specific market type."""
        return [s for s in self.scenarios if s.metrics.market_scenario == market_scenario]