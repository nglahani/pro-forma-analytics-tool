...
class DCFAssumptions:
        ...
        for param_name, param_values in forecast_params:
            ...
            if len(param_values) != 6:
                raise ValidationError(
                    f"{param_name} must have 6 years of data (Years 0-5)"
                )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"DCFAssumptions(scenario={self.scenario_id}, "
...
def validate_monte_carlo_parameters(
    forecasted_parameters: Dict[str, List[float]],
) -> List[str]:
    ...
    for param in required_params:
        if param not in forecasted_parameters:
            ...
            issues.append(
                f"Parameter {param} should have 6 years of data, got {len(forecasted_parameters[param])}"
            )
