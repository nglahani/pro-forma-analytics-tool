...
class InitialNumbers:
    ...
    def _validate_purchase_details(self):
        ...
        expected_cost_basis = self.purchase_price + self.closing_cost_amount + self.renovation_capex
        if abs(self.cost_basis - expected_cost_basis) > 0.01:
            raise ValidationError(f"Cost basis ({self.cost_basis:,.2f}) doesn't match "
    ...
    def _validate_equity_structure(self):
        ...
        if abs(self.total_cash_required - expected_total) > 0.01:
            raise ValidationError(f"Total cash ({self.total_cash_required:,.2f}) doesn't match "
    ...
    def get_operating_expense_breakdown(self) -> Dict[str, float]:
        ...
        return {
            ...
            'expense_ratio': self.total_operating_expenses / self.post_renovation_annual_rent if self.post_renovation_annual_rent > 0 else 0.0
        }
    ...
    def __str__(self) -> str:
        ...
        return (f"InitialNumbers(property={self.property_id}, scenario={self.scenario_id}, "
