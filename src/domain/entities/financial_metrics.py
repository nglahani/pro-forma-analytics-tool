...
class TerminalValue:
    ...
    def _calculate_terminal_value(self):
        ...
        self.gross_property_value = self.final_noi / self.exit_cap_rate if self.exit_cap_rate > 0 else 0.0
        ...
        self.net_sale_proceeds = self.gross_property_value - self.selling_costs_amount - self.remaining_loan_balance
...
class FinancialMetrics:
    ...
    def _validate_metrics(self):
        ...
        if self.internal_rate_return < -1.0 or self.internal_rate_return > 5.0:
            raise ValidationError(f"IRR seems unreasonable: {self.internal_rate_return:.1%}")
    ...
    def get_investment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'terminal_value': self.terminal_value.net_sale_proceeds if self.terminal_value else 0.0,
            'total_return': self.total_distributions + (self.terminal_value.net_sale_proceeds if self.terminal_value else 0.0),
    ...
    def calculate_sensitivity_metrics(self, base_case: 'FinancialMetrics') -> Dict[str, float]:
        """Calculate sensitivity compared to base case."""
        if not base_case:
            return {}
        
        return {
            'npv_variance': ((self.net_present_value - base_case.net_present_value) / abs(base_case.net_present_value)) if base_case.net_present_value != 0 else 0,
            'irr_variance': self.internal_rate_return - base_case.internal_rate_return,
            'equity_multiple_variance': self.equity_multiple - base_case.equity_multiple,
            'payback_variance': self.payback_period_years - base_case.payback_period_years
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FinancialMetrics':
        ...
        if data.get('terminal_value'):
            ...
            terminal_value = TerminalValue(
                ...
                investor_terminal_distribution=tv_data['investor_terminal_distribution'],
                operator_terminal_distribution=tv_data['operator_terminal_distribution']
            )
        ...
        return cls(
            ...
            cash_on_cash_return_year_1=metrics_data.get('cash_on_cash_return_year_1', 0.0),
            ...
            debt_service_coverage_avg=metrics_data.get('debt_service_coverage_avg', 0.0),
            ...
            investment_recommendation=InvestmentRecommendation(analysis_data.get('investment_recommendation', 'HOLD')),
    ...
    def __str__(self) -> str:
        ...
        return (f"FinancialMetrics(property={self.property_id}, scenario={self.scenario_id}, "
