...
class ModelPerformance:
    ...
    def is_acceptable(self, thresholds: Dict[str, float]) -> bool:
        ...
        return (
            self.mae <= thresholds.get("mae", float("inf"))
            and self.mape <= thresholds.get("mape", float("inf"))
            and self.rmse <= thresholds.get("rmse", float("inf"))
            and self.r_squared >= thresholds.get("r_squared", 0.0)
        )
