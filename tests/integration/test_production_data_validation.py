...
class TestProductionDataValidation(unittest.TestCase):
    ...
    def test_interest_rates_data_coverage(self):
        ...
        self.assertGreater(
            len(treasury_data), 50, "Treasury 10Y should have 50+ data points"
        )
        ...
        self.assertGreater(
            len(cmr_data), 50, "Commercial mortgage rate should have 50+ data points"
        )
    ...
    def test_cap_rates_data_coverage(self):
        ...
        self.assertGreaterEqual(
            unique_msas, 3, "Cap rates should cover at least 3 MSAs"
        )
    ...
    def test_rental_market_data_coverage(self):
        ...
        self.assertGreater(
            len(vacancy_data), 100, "Vacancy rates should have 100+ data points"
        )
        ...
        self.assertGreater(
            len(rent_growth_data), 500, "Rent growth should have 500+ data points"
        )
        ...
        self.assertTrue(
            (vacancy_data["value"] >= 0.01).all()
            and (vacancy_data["value"] <= 0.15).all(),
            "Vacancy rates should be between 1% and 15%",
        )
        self.assertTrue(
            (rent_growth_data["value"] >= -0.05).all()
            and (rent_growth_data["value"] <= 0.20).all(),
    ...
    def test_property_growth_data_coverage(self):
        ...
        property_growth_data = pd.read_sql_query(
            "SELECT * FROM property_growth", self.economic_db
        )

        # Test minimum record count
        self.assertGreater(
            len(property_growth_data),
            200,
            "Property growth should have 200+ data points",
        )
        ...
        self.assertGreater(
            date_span, 365 * 8, "Property growth data should span 8+ years"
        )

        # Test reasonable value ranges (-10% to 30%)
        self.assertTrue(
            (property_growth_data["property_growth"] >= -0.10).all()
            and (property_growth_data["property_growth"] <= 0.30).all(),
    ...
    def test_expense_growth_data_coverage(self):
        ...
        expense_data = pd.read_sql_query(
            "SELECT * FROM operating_expenses", self.property_db
        )

        # Test minimum record count
        self.assertGreater(
            len(expense_data), 200, "Expense growth should have 200+ data points"
        )
        ...
        self.assertTrue(
            (expense_data["expense_growth"] >= 0.0).all()
            and (expense_data["expense_growth"] <= 0.15).all(),
    ...
    def test_lending_requirements_data_coverage(self):
        ...
        for param in lending_params:
            with self.subTest(parameter=param):
                ...
                # Test minimum record count
                self.assertGreater(
                    len(data), 200, f"{param} should have 200+ data points"
                )
                ...
                if param == "ltv_ratio":
                    ...
                    self.assertTrue(
                        (data["value"] >= 0.015).all()
                        and (data["value"] <= 0.050).all(),
    ...
    def test_data_temporal_consistency(self):
        ...
        merged = pd.merge(
            treasury_data, cmr_data, on="date", suffixes=("_treasury", "_cmr")
        )
        if len(merged) > 10:
            ...
            self.assertGreater(
                correlation,
                0.7,
                "Commercial mortgage rates should correlate with treasury rates",
            )
    ...
    def test_geographic_variation(self):
        ...
        if len(rent_growth_by_msa) >= 2:
            ...
            self.assertGreater(
                std_deviation, 0.005, "MSAs should show variation in rent growth"
            )
    ...
    def test_complete_parameter_coverage(self):
        ...
        cap_count = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM cap_rates", self.market_db
        ).iloc[0]["count"]
        ...
        property_growth_count = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM property_growth", self.economic_db
        ).iloc[0]["count"]
        parameter_coverage["property_growth"] = property_growth_count > 0

        expense_growth_count = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM operating_expenses", self.property_db
        ).iloc[0]["count"]
        ...
        missing_params = [
            param for param, has_data in parameter_coverage.items() if not has_data
        ]
        self.assertEqual(
            len(missing_params),
            0,
        ...
        self.assertEqual(
            completion_rate, 1.0, "All 11 parameters should have data (100% completion)"
        )
    ...
    def test_business_rule_commercial_mortgage_spread(self):
        ...
        self.assertGreaterEqual(
            min_spread, 0.01, "Commercial mortgage spread should be at least 1%"
        )
        self.assertLessEqual(
            max_spread, 0.05, "Commercial mortgage spread should be at most 5%"
        )
    ...
    def test_business_rule_ltv_ratio_bounds(self):
        ...
        self.assertTrue(
            (ltv_data["value"] >= 0.50).all(), "All LTV ratios should be >= 50%"
        )
        self.assertTrue(
            (ltv_data["value"] <= 0.85).all(), "All LTV ratios should be <= 85%"
        )
    ...
    def test_data_quality_metrics(self):
        ...
        # Market data records
        market_records = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM interest_rates", self.market_db
        ).iloc[0]["count"]
        market_records += pd.read_sql_query(
            "SELECT COUNT(*) as count FROM cap_rates", self.market_db
        ).iloc[0]["count"]
        ...
        property_records = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM rental_market_data", self.property_db
        ).iloc[0]["count"]
        property_records += pd.read_sql_query(
            "SELECT COUNT(*) as count FROM operating_expenses", self.property_db
        ).iloc[0]["count"]
        ...
        economic_records = pd.read_sql_query(
            "SELECT COUNT(*) as count FROM property_growth", self.economic_db
        ).iloc[0]["count"]
        economic_records += pd.read_sql_query(
            "SELECT COUNT(*) as count FROM lending_requirements", self.economic_db
        ).iloc[0]["count"]
        ...
        self.assertGreater(
            total_records,
            2000,
            "Total records should exceed 2000 for production readiness",
        )
