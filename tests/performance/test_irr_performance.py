...
class TestIRRPerformance:
    ...
    def test_irr_calculation_speed(self):
        ...
        for scenario_name, cash_flows in self.test_scenarios.items():
            ...
            print(f"{scenario_name:20} | {avg_time:6.2f}ms | IRR: {avg_irr:6.1%} | Std: {irr_std:.6f}")
        ...
        assert avg_performance < 50.0, f"IRR calculation too slow: {avg_performance:.2f}ms > 50ms"
        
        # Consistency assertions
        for scenario_name, result in results.items():
            assert result['irr_std'] < 1e-10, f"IRR calculation inconsistent for {scenario_name}: std={result['irr_std']}"
    ...
    def test_irr_edge_cases_performance(self):
        ...
        for case_name, cash_flows in edge_cases.items():
            ...
            assert execution_time < 100.0, f"Edge case {case_name} too slow: {execution_time:.2f}ms"
    ...
    def test_irr_batch_performance(self):
        ...
        for i in range(100):
            ...
            annual_cash_flows = np.random.uniform(50000, 500000, 4)   # 4 years of cash flows
            terminal_value = np.random.uniform(800000, 12000000)       # Terminal value
            
            cash_flows = [-initial_investment] + list(annual_cash_flows) + [terminal_value]
        ...
        print(f"Valid IRR count: {sum(1 for irr in irr_results if irr > 0)}/{len(irr_results)}")
        
        # Performance assertions for batch processing
        assert avg_time_per_calculation < 20.0, f"Batch IRR processing too slow: {avg_time_per_calculation:.2f}ms per calculation"
        assert total_time < 5.0, f"Total batch processing time too slow: {total_time:.3f}s"
        
        # Quality assertions - more realistic criteria for random scenarios
        valid_irr_count = sum(1 for irr in irr_results if -0.5 < irr < 3.0)  # Broader reasonable IRR range
        successful_calculations = sum(1 for irr in irr_results if irr != 0.0)  # Non-zero results
        
        print(f"Successful calculations (non-zero): {successful_calculations}/{len(batch_scenarios)}")
        
        # Most calculations should succeed (not return 0.0 due to failure)
        assert successful_calculations > len(batch_scenarios) * 0.7, f"Too many failed IRR calculations: {successful_calculations}/{len(batch_scenarios)}"
    ...
    def test_irr_accuracy_validation(self):
        ...
        for i, cash_flows in enumerate(validation_cases):
            ...
            if irr > 0:  # Only validate for cases where IRR was found
                assert abs(npv_at_irr) < 1e-3, f"NPV at IRR should be ~0, got {npv_at_irr:.6f}"
