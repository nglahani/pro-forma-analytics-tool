...
class TestParameterId:
    ...
    def test_parameter_id_creation_with_empty_name_should_raise_error(self):
        ...
        # Arrange & Act & Assert
        with pytest.raises(ValueError, match="Parameter name and geographic code are required"):
            ParameterId(
                name="",
                geographic_code="35620",
                parameter_type=ParameterType.MARKET_METRIC
...
class TestHistoricalData:
    ...
    def test_historical_data_with_empty_data_points_should_raise_error(self):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Historical data must contain at least one data point"):
            HistoricalData(
                parameter_id=parameter_id,
                data_points=[],
                start_date=date(2023, 1, 1),
    ...
    def test_historical_data_with_inconsistent_date_range_should_raise_error(self):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Date range does not match actual data points"):
            HistoricalData(
                parameter_id=parameter_id,
                data_points=data_points,
...
class TestModelPerformance:
    ...
    def test_model_performance_is_acceptable_with_poor_metrics_should_return_false(self):
