...
class TestInitialNumbersService:
    ...
    def test_calculate_initial_numbers_should_return_valid_initial_numbers(
        self, service, sample_property_data, sample_dcf_assumptions
    ):
        """
        ...
        """
        # Act
        result = service.calculate_initial_numbers(
            sample_property_data, sample_dcf_assumptions
        )
    ...
    def test_calculate_initial_numbers_should_calculate_correct_equity_distribution(
        ...
        assert (
            abs(result.investor_cash_required - total_cash * 0.80) < 1
        )  # Allow rounding
        assert (
            abs(result.operator_cash_required - total_cash * 0.20) < 1
        )  # Allow rounding
        assert (
            abs(
                result.investor_cash_required
                + result.operator_cash_required
                - total_cash
            )
            < 1
        )  # Rounding
    ...
    def test_calculate_initial_numbers_should_handle_no_renovation_scenario(
        ...
        # Act
        result = service.calculate_initial_numbers(
            no_renovation_property, sample_dcf_assumptions
        )
    ...
    def test_calculate_initial_numbers_should_estimate_renovation_cost_when_not_provided(
        ...
        # Act
        result = service.calculate_initial_numbers(
            property_no_cost, sample_dcf_assumptions
        )
        ...
        assert (
            50000 <= result.renovation_capex <= 200000
        )  # Reasonable range for 10 units
    ...
    def test_calculate_initial_numbers_with_missing_purchase_price_should_raise_validation_error(
        ...
        # Act & Assert
        with pytest.raises(
            ValidationError, match="Property purchase price is required"
        ):
            service.calculate_initial_numbers(
                sample_property_data, sample_dcf_assumptions
            )
    ...
    def test_calculate_income_structure_should_account_for_renovation_period(
        ...
        assert (
            abs(result.year_1_rental_income - expected_year_1) < 1000
    ...
    def test_calculate_operating_expenses_should_be_percentage_of_rent(
        ...
        # Assert
        expense_ratio = (
            result.total_operating_expenses / result.post_renovation_annual_rent
        )
    ...
    def test_validate_initial_numbers_should_identify_high_ltv(
        ...
        initial_numbers = service.calculate_initial_numbers(
            sample_property_data, high_ltv_assumptions
        )
    ...
    def test_validate_initial_numbers_should_identify_low_dscr(
        ...
        initial_numbers = service.calculate_initial_numbers(
            sample_property_data, high_rate_assumptions
        )
    ...
    def test_calculate_initial_numbers_should_log_calculation_details(
        self, mock_logger, service, sample_property_data, sample_dcf_assumptions
    ):
        """
        GIVEN valid inputs
        WHEN calculating initial numbers
        ...
        assert any(
            "Successfully calculated initial numbers" in msg for msg in logged_messages
        )
    ...
    def test_calculate_initial_numbers_should_handle_commercial_units(
        ...
        sample_property_data.commercial_units = CommercialUnits(
            total_units=2, average_rent_per_unit=3000
        )
