...
from src.domain.entities.financial_metrics import FinancialMetrics, InvestmentRecommendation, RiskLevel
...
class TestFinancialMetricsService:
    ...
    def test_calculate_financial_metrics_with_negative_cash_flows_should_handle_gracefully(
        ...
        assert result.investment_recommendation in [InvestmentRecommendation.SELL, InvestmentRecommendation.STRONG_SELL]
    ...
    def test_calculate_financial_metrics_with_invalid_inputs_should_raise_validation_error(
        ...
        # Act & Assert
        with pytest.raises(ValidationError, match="Financial metrics calculation failed"):
