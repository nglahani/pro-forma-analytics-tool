...
class TestFinancialMetricsService:
        ...
        for year in range(6):
            if year == 0:
                ...
                distribution = WaterfallDistribution(
                    ...
                    investor_preferred_return_accrued=max(0, preferred_due - preferred_paid),
                    cumulative_unpaid_preferred=0,
    ...
    def test_npv_calculation(self):
        ...
        expected_npv = (-1000 + 200/1.1 + 300/(1.1**2) + 400/(1.1**3) + 500/(1.1**4))
    ...
    def test_investment_recommendation(self):
        ...
        strong_rec, rationale = service._generate_investment_recommendation(
            npv=500000, irr=0.20, equity_multiple=2.5, 
            payback_period=3.0, risk_level=RiskLevel.LOW
        )
        
        assert strong_rec in [InvestmentRecommendation.BUY, InvestmentRecommendation.STRONG_BUY]
        assert len(rationale) > 0
        
        # Poor investment scenario
        poor_rec, rationale = service._generate_investment_recommendation(
            npv=-200000, irr=0.05, equity_multiple=0.8,
            payback_period=8.0, risk_level=RiskLevel.VERY_HIGH
        )
        
        assert poor_rec in [InvestmentRecommendation.SELL, InvestmentRecommendation.STRONG_SELL]
        assert len(rationale) > 0
