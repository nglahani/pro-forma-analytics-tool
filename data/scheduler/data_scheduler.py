...
from data.collectors.data_orchestrator import (
    CollectionJob,
    CollectionResult,
    create_orchestrator,
)
...
class LiveDataScheduler:
    ...
    def __init__(
        self, fred_api_key: Optional[str] = None, config_file: Optional[str] = None
    ):
    ...
    def _load_configuration(self):
        ...
        if config_path.exists():
            try:
                ...
                for param_name, config in config_data.get(
                    "scheduled_updates", {}
                ).items():
                    self.scheduled_updates[param_name] = ScheduledUpdate(
                        ...
                        last_update=(
                            datetime.fromisoformat(config["last_update"])
                ...
                self.logger.info(
                    f"Loaded {len(self.scheduled_updates)} scheduled updates from {config_path}"
                )
    ...
    def _save_configuration(self):
        ...
        for param_name, update in self.scheduled_updates.items():
            config_data["scheduled_updates"][param_name] = {
                ...
                "last_update": (
                    update.last_update.isoformat() if update.last_update else None
                ),
    ...
    def _setup_default_schedules(self):
        ...
        self.logger.info(
            f"Setup {len(self.scheduled_updates)} default scheduled updates"
        )

    def add_scheduled_update(
        self,
        parameter_name: str,
        geographic_codes: List[str],
        frequency: UpdateFrequency,
        ...
        try:
            ...
            self._save_configuration()
            self.logger.info(
                f"Added scheduled update for {parameter_name}: {frequency.value} at {time_of_day}"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Failed to add scheduled update for {parameter_name}: {e}"
            )
            return False
    ...
    def _execute_parameter_update(self, parameter_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if failed_jobs:
                ...
                update_config.last_error = (
                    f"{len(failed_jobs)} of {len(jobs)} jobs failed"
                )
            else:
            ...
            self.logger.info(
                f"Completed update for {parameter_name}: {len(successful_jobs)}/{len(jobs)} jobs successful, {total_records} records"
            )
    ...
    def execute_all_updates(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Completed all updates: {total_successful} successful, {total_failed} failed"
        )
    ...
    def start_scheduler(self):
        ...
        for param_name, update_config in self.scheduled_updates.items():
            ...
            if update_config.frequency == UpdateFrequency.DAILY:
                ...
                schedule.every().monday.at(update_config.time_of_day).do(
                    update_func
                )  # Monday updates
                ...
                schedule.every().month.do(
                    update_func
                )  # Will need custom logic for quarterly
        ...
        self.logger.info(
            f"Scheduler started with {len(self.scheduled_updates)} configured updates"
        )
        ...
        try:
            ...
            if stale_parameters:
                ...
                for param_report in stale_parameters:
                    self.logger.warning(
                        f"  {param_report.parameter_name} ({param_report.geographic_code}): {param_report.days_since_update} days old"
                    )
            else:
        ...
        for param_name, update_config in self.scheduled_updates.items():
            for geo_code in update_config.geographic_codes:
                try:
                    ...
                    self.logger.error(
                        f"Failed to check freshness for {param_name} ({geo_code}): {e}"
                    )
    ...
    def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_updates": len(
                [u for u in self.scheduled_updates.values() if u.enabled]
            ),
                ...
                next_jobs.append(
                    {
                        ...
                        "last_update": (
                            update_config.last_update.isoformat()
...
def create_scheduler(
    fred_api_key: Optional[str] = None, config_file: Optional[str] = None
) -> LiveDataScheduler:
