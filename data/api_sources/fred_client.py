...
class FredAPIClient:
    ...
    def get_series_data(self, series_id: str, start_date: Optional[date] = None, 
    ...
    def get_interest_rate_data(self, start_date: Optional[date] = None,
                              end_date: Optional[date] = None) -> Dict[str, pd.DataFrame]:
        ...
        return self.get_multiple_series(
            list(interest_rate_series.values()), 
            start_date, 
            end_date
        )
    
    def get_economic_indicators(self, start_date: Optional[date] = None,
                               end_date: Optional[date] = None) -> Dict[str, pd.DataFrame]:
...
class FredDataCollector:
    ...
    def collect_parameter_data(self, parameter_names: Optional[List[str]] = None,
                              start_date: Optional[date] = None,
                              end_date: Optional[date] = None) -> Dict[str, pd.DataFrame]:
        """
            ...
            if param_def and param_def.fred_series:
                try:
                    ...
                    if not data.empty:
                        ...
                        self.logger.info(f"Collected {len(data)} data points for {param_name}")
    ...
    def validate_data_quality(self, data: pd.DataFrame, parameter_name: str) -> Dict[str, Any]:
        ...
        if param_def:
            ...
            if out_of_range > 0:
                ...
                quality_metrics['issues'].append(f"{out_of_range} values outside expected range [{min_val}, {max_val}]")
        ...
        if len(data) > 1:
            ...
            if large_gaps > 0:
                quality_metrics['issues'].append(f"{large_gaps} gaps larger than expected in time series")
