...
class FredEnhancedCollector(BaseDataCollector):
    ...
    def collect_data(self, parameter_name: str, geographic_code: str,
        ...
        self.logger.info(f"Collected {len(df)} observations for {parameter_name} from {start_date} to {end_date}")
    ...
    def collect_multiple_parameters(self, parameter_names: List[str],
        ...
        for param_name in parameter_names:
            ...
            try:
                ...
                # Log success
                self.logger.info(f"Successfully collected {len(data)} records for {param_name}")
...
def create_fred_collector(api_key: Optional[str] = None) -> FredEnhancedCollector:
    ...
    if not api_key:
        raise ValueError("FRED API key must be provided or set in environment variable FRED_API_KEY")
