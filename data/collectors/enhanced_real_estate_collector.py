...
class EnhancedRealEstateCollector(BaseDataCollector):
    ...
    def __init__(self, fred_api_key: Optional[str] = None):
        ...
        self.fred_collector = create_fred_collector(fred_api_key) if fred_api_key else None
    ...
    def collect_data(self, parameter_name: str, geographic_code: str,
        ...
        if parameter_name == 'cap_rate':
            ...
            raise ValueError(f"Parameter {parameter_name} not supported by this collector")
    
    def _calculate_cap_rates(self, geographic_code: str, start_date: date, end_date: date) -> pd.DataFrame:
        """Calculate cap rates using treasury rates + risk premium approach."""
        
        if not self.fred_collector:
            self.logger.warning("FRED collector not available - cannot calculate cap rates")
            return pd.DataFrame()
        ...
        try:
            ...
            if treasury_data.empty:
                self.logger.warning("No treasury data available for cap rate calculation")
                return pd.DataFrame()
            ...
            for _, row in treasury_data.iterrows():
                ...
                final_cap_rate = max(0.03, adjusted_cap_rate + market_noise)  # Floor at 3%
            ...
            self.logger.info(f"Calculated {len(result_df)} cap rate observations for {geographic_code}")
            return result_df
    ...
    def _calculate_vacancy_rates(self, geographic_code: str, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        data_points = []
        current_date = start_date.replace(month=((start_date.month-1)//3)*3+1, day=1)  # Start of quarter
        ...
        while current_date <= end_date:
            ...
            seasonal_adjustment = 0.005 * np.sin(2 * np.pi * (current_date.month - 4) / 12)
            
            # Market-specific noise
            market_noise = np.random.normal(0, 0.01)
            
            adjusted_vacancy = baseline_vacancy + cycle_adjustment + seasonal_adjustment + market_noise
            adjusted_vacancy = max(0.02, min(0.15, adjusted_vacancy))  # Bound between 2% and 15%
        ...
        self.logger.info(f"Calculated {len(result_df)} vacancy rate observations for {geographic_code}")
        return result_df
    ...
    def validate_api_connection(self) -> bool:
        ...
        if self.fred_collector:
            ...
            self.logger.warning("FRED collector not available - cap rate calculation will fail")
            return False


def create_enhanced_real_estate_collector(fred_api_key: Optional[str] = None) -> EnhancedRealEstateCollector:
