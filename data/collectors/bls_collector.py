...
class BLSDataCollector(BaseDataCollector):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.api_key = (
            api_key  # Optional - public API allows 25 requests/day without key
        )
    ...
    def collect_data(
        ...
        if not series_id:
            self.logger.warning(
                f"No BLS series available for {parameter_name} in {geographic_code}"
            )
            return pd.DataFrame()
        ...
        if not self.api_key and (end_year - start_year) > 19:
            ...
            self.logger.warning(
                f"Limited to {start_year}-{end_year} for unregistered BLS API"
            )
        ...
        for item in series_data.get("data", []):
            try:
                ...
                self.logger.warning(
                    f"Skipping invalid BLS observation: {item}, error: {e}"
                )
                continue
        ...
        self.logger.info(
            f"Collected {len(df)} observations for {parameter_name} from BLS"
        )
        return df

    def _get_series_id(
        self, parameter_name: str, geographic_code: str
    ) -> Optional[str]:
        ...
        if "NC" in msa_info.state:
            return regional_series.get(
                "southeast", self.BLS_SERIES_MAP[parameter_name]["national"]
            )
    ...
    def _calculate_growth_rates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["growth_rate"] = (df["value"] - df["prev_year_value"]) / df[
            "prev_year_value"
        ]
