...
class FHFACollector(BaseDataCollector):
    ...
    def collect_data(
        ...
        try:
            ...
            # Filter date range
            msa_data = msa_data[
                (msa_data["date"] >= start_date) & (msa_data["date"] <= end_date)
            ]

            if msa_data.empty:
                self.logger.warning(
                    f"No FHFA data in date range {start_date} to {end_date}"
                )
                return pd.DataFrame()

            # Calculate year-over-year growth rate from index
            msa_data = msa_data.sort_values("date")
            msa_data["prev_year_index"] = msa_data["index_sa"].shift(
                4
            )  # 4 quarters ago
            ...
            msa_data["commercial_growth"] = (
                msa_data["growth_rate"] * commercial_multiplier
            )
            ...
            self.logger.info(
                f"Collected {len(result_df)} property growth observations from FHFA"
            )
            return result_df
    ...
    def validate_api_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"FHFA connection failed: HTTP {response.status_code}"
                )
                return False
