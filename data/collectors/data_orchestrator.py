...
from .enhanced_real_estate_collector import EnhancedRealEstateCollector, create_enhanced_real_estate_collector
...
class DataOrchestrator:
    ...
    PARAMETER_DB_MAPPING = {
        ...
        'commercial_mortgage_rate': {'db_name': 'market_data', 'table_name': 'interest_rates'},
        ...
        'vacancy_rate': {'db_name': 'property_data', 'table_name': 'rental_market_data'},
        'rent_growth': {'db_name': 'property_data', 'table_name': 'rental_market_data'},
        
        # Operating expenses -> property_data.operating_expenses
        'expense_growth': {'db_name': 'property_data', 'table_name': 'operating_expenses'},
        
        # Property growth -> economic_data.property_growth
        'property_growth': {'db_name': 'economic_data', 'table_name': 'property_growth'},
        
        # Lending requirements -> economic_data.lending_requirements
        'ltv_ratio': {'db_name': 'economic_data', 'table_name': 'lending_requirements'},
        'closing_cost_pct': {'db_name': 'economic_data', 'table_name': 'lending_requirements'},
        'lender_reserves': {'db_name': 'economic_data', 'table_name': 'lending_requirements'},
    }
    ...
    def _setup_collectors(self, fred_api_key: Optional[str]):
        ...
        try:
            ...
            if fred_api_key:
                ...
                self.logger.warning("FRED API key not available - skipping FRED collector")
    ...
    def get_collection_plan(self, start_date: date, end_date: date, 
                          msas: Optional[List[str]] = None) -> List[CollectionJob]:
        """
            ...
            for msa in msas:
                if 'Enhanced_RealEstate' in self.collectors:
                    jobs.append(CollectionJob(
                        ...
                        database_info=self.PARAMETER_DB_MAPPING.get(param, {'db_name': 'market_data', 'table_name': 'cap_rates'}),
                        expected_range=expected_range
                    ))
            ...
            for msa in msas:
                if 'BLS' in self.collectors:
                    jobs.append(CollectionJob(
                        ...
                        end_date=end_date,
                        collector_name='BLS',
                        database_info=self.PARAMETER_DB_MAPPING.get(param, {'db_name': 'property_data', 'table_name': 'rental_market_data'}),
                        expected_range=expected_range
                    ))
            ...
            for msa in msas:
                if 'FHFA' in self.collectors:
                    jobs.append(CollectionJob(
                        ...
                        end_date=end_date,
                        collector_name='FHFA',
                        database_info=self.PARAMETER_DB_MAPPING.get(param, {'db_name': 'economic_data', 'table_name': 'property_growth'}),
                        expected_range=expected_range
                    ))
    ...
    def execute_collection_job(self, job: CollectionJob) -> CollectionResult:
        ...
        try:
            self.logger.info(f"Executing job: {job.parameter_name} for {job.geographic_code}")
            ...
            self.logger.error(f"Job failed: {job.parameter_name} for {job.geographic_code}: {e}")
            return CollectionResult(
                job=job,
                success=False,
                records_collected=0,
    ...
    def execute_collection_plan(self, jobs: List[CollectionJob], 
                              max_workers: int = 4) -> List[CollectionResult]:
        """
        ...
        self.logger.info(f"Executing {len(jobs)} collection jobs with {max_workers} workers")
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all jobs
            future_to_job = {
                executor.submit(self.execute_collection_job, job): job 
                for job in jobs
            }
            ...
            for future in as_completed(future_to_job):
                try:
                    ...
                    if result.success:
                        self.logger.info(f"SUCCESS {result.job.parameter_name} ({result.job.geographic_code}): {result.records_collected} records")
                    else:
                        self.logger.error(f"FAILED {result.job.parameter_name} ({result.job.geographic_code}): {result.error_message}")
                        
                except Exception as e:
                    job = future_to_job[future]
                    self.logger.error(f"Job execution failed: {job.parameter_name}: {e}")
    ...
    def replace_all_mock_data(self, start_date: Optional[date] = None, 
        ...
        self.logger.info(f"Starting complete mock data replacement from {start_date} to {end_date}")
        ...
        results = self.execute_collection_plan(jobs, max_workers=3)  # Conservative parallelism
        
        # Step 4: Generate summary
        summary = self._generate_collection_summary(results, backup_paths)
        
        self.logger.info(f"Mock data replacement completed. Success rate: {summary['success_rate']:.1%}")
        
        return summary
    
    def _generate_collection_summary(self, results: List[CollectionResult], 
                                   backup_paths: List[str]) -> Dict[str, Any]:
        ...
        for param, db_info in self.PARAMETER_DB_MAPPING.items():
            try:
                ...
                if param in ['treasury_10y', 'fed_funds_rate', 'commercial_mortgage_rate']:
                    ...
                    data_points = db_manager.get_parameter_data(param, '35620')  # Sample MSA
...
if __name__ == "__main__":
    ...
    for result in results:
        ...
        print(f"{status} {result.job.parameter_name} ({result.job.geographic_code}): {result.records_collected} records")